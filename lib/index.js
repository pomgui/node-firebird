...
Database.prototype.getInfoSql = function(request, callback) {
    this.connection.getInfoSql(request, callback);
}
...
Connection.prototype.getInfoSql = function(requestedItems) {
	var self = this;
	var mainCallback = function(err, ret) {
		if (!err) {
			mainCallback.response.handle = ret.handle;
			describe(ret.buffer, mainCallback.response);
			self.db.emit('getInfoSql', requestedItems);
			ret = mainCallback.response;
		}
    ...
    msg.addInt(this.dbhandle);
    msg.addInt(0); // IncarnationOfObject?

	blr.addBytes(requestedItems);
	msg.addInt(0xFFFF);
